<div align="center">

<img src="https://img.shields.io/badge/Whizard-Skills-7c3aed?style=for-the-badge&logo=sparkles&logoColor=white" alt="badge">

<h1>Whizard Skills</h1>

<p><em>AI 编程代理技能合集</em></p>

<p>
  <img src="https://img.shields.io/badge/Codex-Ready-10a37f?style=flat-square&logo=openai&logoColor=white" alt="Codex">
  <img src="https://img.shields.io/badge/Claude_Code-Ready-d97757?style=flat-square&logo=anthropic&logoColor=white" alt="Claude">
  <img src="https://img.shields.io/badge/OpenCode-Ready-22c55e?style=flat-square&logo=terminal&logoColor=white" alt="OpenCode">
  <img src="https://img.shields.io/badge/7-Skills-7c3aed?style=flat-square&logo=sparkles&logoColor=white" alt="Skills">
</p>

<p>
  <a href="README.md"><img src="https://img.shields.io/badge/中文-当前-red?style=for-the-badge&labelColor=dc2626" alt="中文"></a>
  <a href="README_en.md"><img src="https://img.shields.io/badge/English-blue?style=for-the-badge" alt="English"></a>
</p>

<br>

---

</div>

## 技能合集

<table>
<tr>
<td align="center" width="25%">

<a href="./awesome-design-md/">

<img src="https://img.shields.io/badge/ awesome--design--md-7c3aed?style=for-the-badge&logo=figma&logoColor=white" alt="awesome-design-md">

</a>

**73+ 品牌设计系统**

Stripe · Apple · Tesla · Spotify

---

让 AI 生成一致的、符合品牌风格的 UI

<small>

[`查看技能`](./awesome-design-md/) · [`品牌目录`](./awesome-design-md/references/collection-catalog.md)

</small>

</td>
<td align="center" width="25%">

<a href="./humanizer/">

<img src="https://img.shields.io/badge/humanizer-059669?style=for-the-badge&logo=pen-fancy&logoColor=white" alt="humanizer">

</a>

**33 种 AI 模式检测**

自然 · 人性化 · 无痕迹

---

去除 AI 写作痕迹，匹配你的个人风格

<small>

[`查看技能`](./humanizer/) · [`模式示例`](./humanizer/references/pattern-examples.md)

</small>

</td>
<td align="center" width="25%">

<a href="./ponytail/">

<img src="https://img.shields.io/badge/ponytail-2563eb?style=for-the-badge&logo=cut&logoColor=white" alt="ponytail">

</a>

**代码量 -54%**

YAGNI · 极简 · 高效

---

强制执行 YAGNI，成本降 20%，速度提 27%

<small>

[`查看技能`](./ponytail/)

</small>

</td>
<td align="center" width="25%">

<a href="./rtk/">

<img src="https://img.shields.io/badge/rtk-dc2626?style=for-the-badge&logo=terminal&logoColor=white" alt="rtk">

</a>

**Token -60~90%**

紧凑 · 优化 · 快速

---

CLI 代理，支持 git/docker/AWS 等 14+ 工具

<small>

[`查看技能`](./rtk/)

</small>

</td>
</tr>
<tr>
<td align="center" width="25%">

<a href="./matt-pocock-engineering/">

<img src="https://img.shields.io/badge/matt--pocock--engineering-0891b2?style=for-the-badge&logo=hammer&logoColor=white" alt="matt-pocock-engineering">

</a>

**17+ 工程技能**

对齐 · TDD · 设计

---

专业的 AI 辅助开发工程实践

<small>

[`查看技能`](./matt-pocock-engineering/)

</small>

</td>
<td align="center" width="25%">

<a href="./ui-layouts/">

<img src="https://img.shields.io/badge/ui--layouts-d946ef?style=for-the-badge&logo=react&logoColor=white" alt="ui-layouts">

</a>

**80+ 创意组件**

动画 · 视觉 · 交互

---

Tailwind + Framer Motion 即用型组件

<small>

[`查看技能`](./ui-layouts/) · [`组件目录`](./ui-layouts/references/components.md)

</small>

</td>
<td align="center" width="25%">

<a href="./gsap/">

<img src="https://img.shields.io/badge/gsap-88ce02?style=for-the-badge&logo=javascript&logoColor=black" alt="gsap">

</a>

**专业级动画**

滚动 · SVG · 文本 · 时间轴

---

行业标准 JS 动画库，高性能跨浏览器

<small>

[`查看技能`](./gsap/) · [`插件参考`](./gsap/references/plugins.md)

</small>

</td>
<td align="center" width="25%">

<!-- Placeholder for future skill -->

</td>
</tr>
</table>

---

## 快速安装

<div align="center">

```bash
# 一键安装所有技能
git clone https://github.com/wizenard/whizard_skills.git ~/.codex/skills
```

<img src="https://img.shields.io/badge/支持-Codex_%7C_Claude_%7C_OpenCode-7c3aed?style=flat-square" alt="support">

</div>

<details>
<summary><b>📦 只安装特定技能</b></summary>

<br>

```bash
git clone https://github.com/wizenard/whizard_skills.git /tmp/whizard_skills

# 选择你需要的技能
cp -r /tmp/whizard_skills/awesome-design-md ~/.codex/skills/
cp -r /tmp/whizard_skills/humanizer ~/.codex/skills/
cp -r /tmp/whizard_skills/ponytail ~/.codex/skills/
cp -r /tmp/whizard_skills/rtk ~/.codex/skills/
cp -r /tmp/whizard_skills/matt-pocock-engineering ~/.codex/skills/
cp -r /tmp/whizard_skills/ui-layouts ~/.codex/skills/
cp -r /tmp/whizard_skills/gsap ~/.codex/skills/
```

</details>

---

## 使用示例

<table>
<tr>
<td width="20%" align="center">

`🎨 设计系统`

</td>
<td width="80%">

```bash
Use $awesome-design-md to help me build a landing page like Stripe
```

</td>
</tr>
<tr>
<td align="center">

`✍️ 人性化`

</td>
<td>

```bash
Use $humanizer to rewrite this AI-generated text to sound more natural
```

</td>
</tr>
<tr>
<td align="center">

`🪡 精简代码`

</td>
<td>

```bash
Use $ponytail to write minimal code for this feature
```

</td>
</tr>
<tr>
<td align="center">

`⚡ 优化命令`

</td>
<td>

```bash
Use $rtk to optimize my command output
```

</td>
</tr>
<tr>
<td align="center">

`🏗️ 工程实践`

</td>
<td>

```bash
Use $matt-pocock-engineering to show me the TDD workflow
```

</td>
</tr>
<tr>
<td align="center">

`🎭 动画组件`

</td>
<td>

```bash
Use $ui-layouts to create creative animated buttons
```

</td>
</tr>
<tr>
<td align="center">

`🎬 GSAP 动画`

</td>
<td>

```bash
Use $gsap to create a scroll-triggered animation with ScrollTrigger
```

</td>
</tr>
</table>

---

<div align="center">

## 技能统计

<table>
<tr>
<td align="center"><b>7</b><br>技能数量</td>
<td align="center"><b>73+</b><br>品牌设计系统</td>
<td align="center"><b>80+</b><br>React 组件</td>
<td align="center"><b>33</b><br>AI 模式检测</td>
<td align="center"><b>20+</b><br>GSAP 插件</td>
</tr>
</table>

<br>

---

<br>

### 📄 许可证

MIT License © [wizenard](https://github.com/wizenard)

<br>

<img src="https://img.shields.io/badge/Made_with-💜-purple?style=for-the-badge" alt="Made with love">

<img src="https://komarev.com/ghpvc/?username=wizenard&label=Profile%20Views&color=7c3aed&style=for-the-badge" alt="views">

</div>
